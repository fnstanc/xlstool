...
def parse_fields(sheet, group):
    ...
    for i in range(DATA_BEGIN_COL, ncols):
        ...
        if not field_map.has_key(field_name):
            field_names.append(field_name)
            field_map[field_name] = {
                "field_name": field_name,
                "field_type": field_type,
                "cols": [i],
                "desc": sheet.cell_value(FIELD_COMMENT_ROW, i).encode("utf-8"),
            }
        else:
            ...
            field_info["cols"].append(i)
            field_map[field_name] = field_info

    return field_names, field_map


def output_proto_header(content, file_name):
    content.append("/**\n")
    content.append("* @file: " + file_name + "\n")
    content.append("* @brief: This file is generated by xlstool, please don't edit it.\n")
    content.append("*/\n")
    content.append("\n")
...
def output_id_filed(content):
    ...
    output_field(field_info, 1, content)


def gen_proto(sheet_name, field_names, field_map):
    file_name = sheet_name + ".proto"
    proto_file = os.path.join(PROTO_OUTPUT_PATH, file_name)
    ...
    for field_name in field_names:
        output_field(field_map[field_name], field_index, content)
        field_index += 1
...
def gen_binary(sheet, sheet_name, field_names, field_map):
    ...
    for row in range(DATA_BEGIN_ROW, sheet.nrows):
        ...
        parse_row(sheet, row, item_id, field_names, field_map, item)
        index += 1
...
def process_one_file(xls_file_path, output, group):
    ...
    for name in sheet_names:
        ...
        field_names, field_map = parse_fields(sheet, group)
        if len(field_names) == 0:
            continue

        gen_proto(sheet_name, field_names, field_map)
